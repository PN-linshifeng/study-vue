<template>
  <div v-highlight>
    ...
    <div class="margin-block">
      ...
      <p class="notes">{{$route.params.id?$route.params.id+"的性别是："+sex:"点击链接"}}</p>
      <p>
        复用组件时，生命周期钩子不会再被调用,想对路由参数的变化作出响应的话，你可以简单地
        <span class="strong">watch (监测变化) $route</span> 对象或者使用
        <span class="strong">beforeRouteUpdate</span>
      </p>
      <p>HTML代码：</p>
    ...
    <div class="margin-block">
      ...
      <p>
        ...
        <a
          ...
          target="—blank"
        >path-to-regexp</a> 作为路径匹配引擎，所以支持很多高级的匹配模式，例如：可选的动态路径参数、匹配零个或多个、一个或多个，甚至是自定义正则匹配。查看它的 文档 学习高阶的路径匹配，还有
        <a
          href="https://github.com/vuejs/vue-router/blob/next/examples/route-matching/app.js"
          target="—blank"
        >这个例子 展示 vue-router</a> 怎么使用这类匹配。
      </p>
    </div>
    <div class="margin-block">
      <h2>匹配优先级</h2>
